...
import src.otus_hw4.scoring as scoring
...
def online_score_request(request: MethodRequest, ctx, store):
    """Запрос скоринга."""
...
def clients_interest_request(request: MethodRequest, ctx, store):
    """Запрос интересов."""
...
def method_request(body, ctx, store):
    """Запрос к API."""
    ...
    # Запрос скоринга
        ...
        response, code = online_score_request(method_req, ctx, store)
    # Запрос увлечений
    elif method_req.method == "clients_interests":
        response, code = clients_interest_request(method_req, ctx, store)
    # Неизвестный запрос
    else:
        code = INVALID_REQUEST
